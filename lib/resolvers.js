...
function prefixResolve(module) {
  ...
  var prefix = process.env.REQUIREG_PREFIX || execSync('npm config get prefix', {encoding:'utf8'}).trim()
...
function yarnModulesResolve(module) {
  ...
  if (!result.error && result.stdout) {
    var yarnPath = process.env.REQUIREG_PREFIX || result.stdout.replace(/[\r\n]+/g, '');
